...
include'layout/identifier.php';
	...
	if(isset($_POST['ON'])) { 
					$update = mysqli_query($kon, "UPDATE `user_alert_E2A` SET `status`=1");	
				}
	else if (isset($_POST['OFF'])) {
					$update = mysqli_query($kon, "UPDATE `user_alert_E2A` SET `status`=0");
				}
	else $update = "";
	...
	if(isset($_POST['cari']) ){
		...
		$que = mysqli_query($kon,"Select * from user_alert_E2A where $kategori like '%$index%' order by nama asc limit 0,1000");
	}
	else {
		$que = mysqli_query($kon,"Select * from user_alert_E2A  order by nama asc limit 0,1000");
	}
	$managemen_active_1='active';
	$index_active_2='active';
?>
...
?>
                      <tr>
                        ...
                        <td><a class="default-button" href="edit_nam.php?msisdn=<?php echo $ambil1['msisdn']; ?>&nama=<?php echo $ambil1['nama']; ?>&grup=<?php echo $ambil1['grup'];?>&status=<?php echo $ambil1['status'];?>&email=<?php echo $ambil1['email'];?>&fungsi=<?php echo $ambil1['fungsi'];?> ">Update</a></td>
                        <td>
						<form action="edited_nam.php?msisdn=<?php echo $ambil1['msisdn']; ?>&nama=<?php echo $ambil1['nama']; ?>&grup=<?php echo $ambil1['grup'];?>&status=<?php echo $ambil1['status'];?>&email=<?php echo $ambil1['email'];?>&fungsi=<?php echo $ambil1['fungsi'];?> " method="post">
